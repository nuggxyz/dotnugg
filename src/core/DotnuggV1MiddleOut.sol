...
function middleout(uint256[][] memory files) pure returns (uint256[] memory res) {
    Run memory run;
    ...
    for (uint8 i = 0; i < run.versions.length; i++) {
        if (run.versions[i].length > 0) {
            ...
            if (run.versions.length != 1) postionForCanvas(run.canvas, run.mix);
...
function center() pure returns (Coordinate memory) {
...
function postionForCanvas(Canvas memory canvas, Mix memory mix) pure {
...
function formatForCanvas(Canvas memory canvas, Mix memory mix) pure {
    Anchor memory receiver = canvas.receivers[mix.feature];
    Anchor memory anchor = mix.version.anchor;
...
function pickVersionIndex(Canvas memory canvas, Parser.Memory[] memory versions) pure returns (uint8) {
...
function checkRluds(Rlud memory r1, Rlud memory r2) pure returns (bool) {
...
function updateReceivers(Canvas memory canvas, Mix memory mix) pure {
...
function mergeToCanvas(Canvas memory canvas, Mix memory mix) pure {
...
function convertReceiversToAnchors(Mix memory mix) pure {
...
function calculateReceiverCoordinate(
    ...
    Coordinate[] memory anchors
) pure returns (Coordinate memory coordinate) {
...
function getAnchors(Matrix.Memory memory matrix) pure returns (Coordinate[] memory anchors) {
