...
import {IDotnuggV1File} from "../interfaces/IDotnuggV1File.sol";
...
library DotnuggV1Storage {
    ...
    function location(uint8 feature) internal view returns (IDotnuggV1File) {
        ...
        return IDotnuggV1File(address(uint160(uint256(proxy))));
    }
    ...
    function read(uint8 feature, uint8 index) internal view returns (uint256[] memory res) {
        address loc = address(location(feature));

        uint8 len = uint8(loc.code[DOTNUGG_RUNTIME_BYTE_LEN]);

        uint32 starts = uint32(bytes4(readBytecode(loc, DOTNUGG_RUNTIME_BYTE_LEN + len * 2 + 1 + index * 2, 4)));
    ...
    function weights(uint8 feature) internal view returns (bytes memory res) {
        address loc = address(location(feature));
        uint8 len = uint8(loc.code[DOTNUGG_RUNTIME_BYTE_LEN]);
    ...
    function search(uint8 feature, uint256 seed) internal view returns (uint8 res) {
        seed = uint256(keccak256(abi.encodePacked(seed, feature, address(this))));

        bytes memory w = weights(feature);
    ...
    function readBytecodeAsArray(
        ...
        assembly {
            ...
            extcodecopy(pointer, add(add(data, 32), offset), start, size)
        }
    }
